...
func TestNewCoreFormat(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		core := newCore(ws, tc.format, ISO8601TimeFormat, LevelDebug)
...
func TestLockedMultiCoreAddCore(t *testing.T) {
	...
	buf1 := &bytes.Buffer{}
	core1 := newCore(zapcore.AddSync(buf1), PlaintextOutput, ISO8601TimeFormat, LevelDebug)
	mc.AddCore(core1)

	buf2 := &bytes.Buffer{}
	core2 := newCore(zapcore.AddSync(buf2), ColorizedOutput, ISO8601TimeFormat, LevelDebug)
	mc.AddCore(core2)
